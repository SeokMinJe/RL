...
# hyperparameters
EPISODES = 1000   # 애피소드(총 플레이할 게임 수) 반복횟수
...
def learn():
    ...
    # Tensor list
    states = torch.stack(states) # [ 0, 1, 1, 0, 0,]
    # print(states)
    # print(states)
    ...
    ''' DQN'''
    # max_next_q = policy_net(next_states).max(1)[0].unsqueeze(1) # 에이전트가 보는 행동의 미래 가치(max_next_q)
    # unsqueeze(): 1인 차원을 생성
